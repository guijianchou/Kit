...
namespace Microsoft.Settings.UITests
{
    ...
    public class OOBEUITests : UITestBase
    {
        ...
        private const string LocalAppDataFolderPath = "%localappdata%\\Kit";
        private const string LastVersionFilePath = "%localappdata%\\Kit\\last_version.txt";
        ...
        {
            // Clean up previous Kit runtime data to simulate first start
            // CleanPowerToysData();
        ...
        */
        ...
        private void CleanPowerToysData()
        {
            ...
            try
            {
                ...
                // Delete Kit runtime folder in LocalAppData
                string kitFolder = Environment.ExpandEnvironmentVariables(LocalAppDataFolderPath);
                if (Directory.Exists(kitFolder))
                {
                    Directory.Delete(kitFolder, true);
                }
            ...
            {
                Assert.Inconclusive($"Could not clean Kit runtime data: {ex.Message}");
            }
        }
        ...
        private void ModifyLastVersionFile()
        {
            try
            {
                // Create Kit runtime folder if it doesn't exist
                string kitFolder = Environment.ExpandEnvironmentVariables(LocalAppDataFolderPath);
                if (!Directory.Exists(kitFolder))
                {
