...
The sparse package lives under `src/PackageIdentity`. It produces a payload-free MSIX whose `Identity` is `Local.Kit.SparseApp` so Kit can register side by side with official PowerToys. The manifest is intentionally narrow and currently declares the Settings UI entry that has a corresponding Kit build output.
...
The script determines the proper `makeappx.exe` for the host build machine (x64 on typical developer boxes) and creates `KitSparse.msix` in `{repo}\<Platform>\<Configuration>`.

> After packaging finishes, the helper also emits `src/PackageIdentity/.user/KitSparse.publisher.txt`. This file mirrors the publisher string Windows will see once the sparse package is registered, which downstream projects can read to stay in sync when generating their own manifests.
...
1. Artifacts are stored in `src/PackageIdentity/.user/KitSparse.certificate.sample.*` (`.cer` and `.thumbprint`).
2. Install the `.cer` into `CurrentUser` → `TrustedPeople` (and `TrustedRoot`, if necessary) so Windows trusts the signature:

   ```powershell
   $repoRoot = "C:/git/Kit"
   Import-Certificate -FilePath "$repoRoot/src/PackageIdentity/.user/KitSparse.certificate.sample.cer" -CertStoreLocation Cert:\CurrentUser\TrustedPeople
   ```
...
After `KitSparse.msix` is generated:

```powershell
# First time registration
$repoRoot = "C:/git/Kit"
$outputRoot = Join-Path $repoRoot "x64/Release"
Add-AppxPackage -Path (Join-Path $outputRoot "KitSparse.msix") -ExternalLocation $outputRoot

# Re-register after manifest tweaks only; the script writes the publisher-adjusted manifest here for local builds
Add-AppxPackage -Register (Join-Path $repoRoot "src/PackageIdentity/.user/KitSparse.AppxManifest.xml") -ExternalLocation $outputRoot -ForceApplicationShutdown

# Remove the sparse identity
Get-AppxPackage -Name Local.Kit.SparseApp | Remove-AppxPackage
```
...
3. Embed a sparse identity manifest in the Win32 binary so it binds to the MSIX identity at runtime. The manifest must declare an `<msix>` element with `packageName="Local.Kit.SparseApp"`, `applicationId` matching the `<Application Id>`, and a `publisher` that matches the sparse package. Keep the manifest's publisher in sync with `src/PackageIdentity/.user/KitSparse.publisher.txt` (emitted by `BuildSparsePackage.ps1`).
...
5. To launch the Win32 surface with identity, use the `shell:AppsFolder` activation form with the package family name and the matching application Id, or activate it via `IApplicationActivationManager::ActivateApplication` using the same AppUserModelID.
   ...
   - For locally built packages, resolve the `<PackageFamilyName>` with `Get-AppxPackage -Name Local.Kit.SparseApp | Select-Object -ExpandProperty PackageFamilyName`.
   - Local developer builds created by this script typically use a package family name derived from the dev certificate.
