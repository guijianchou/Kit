...
- The inactive AdvancedPaste-only `LanguageModelProvider` source tree was deleted instead of only being removed from build graphs. Its AI provider package pins, provider UI metadata/helpers, stale Foundry Local UI string, and stale `OpenAI` third-party notice entry were removed with it, while historical settings serialization models remain for old JSON compatibility.
