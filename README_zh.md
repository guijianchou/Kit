...
- 发布构建仅保留 `en-US` 卫星资源，从运行时输出中删除生成的调试符号和本机链接工件，清理非活动 Settings 模块资产、图标、资源字符串、OOBE/模型资产以及陈旧的非活动控件 XBF 输出，并且不再为活动 Kit 模块集保留仅 AdvancedPaste 的 `LanguageModelProvider` 源码树或 AI provider 包 pin。
