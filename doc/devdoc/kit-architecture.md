...
- 插件兼容：插件契约 = KitModuleIface + kit_create()，官方模块全量现代化适配、第三方按契约开发；
...
- 插件接口 DLL（实现契约）与可选附属进程（如 Awake 的 Kit.Awake.exe，LightSwitch 的 Kit.LightSwitchService.exe）。

进程间通信沿用：TwoWayPipeMessageIPC 命名管道（kit_runner_<uuid> / kit_settings_<uuid>），消息为 JSON。

## 3. 插件系统（本文核心）

### 3.1 契约

- 接口头：src/modules/interface/kit_module_interface.h；
- 工厂：extern "C" __declspec(dllexport) KitModuleIface* __cdecl kit_create()；
...
- 设置 UI 描述：get_config() 输出 KitSettings::Settings 序列化 JSON（properties / links / custom_actions）。

### 3.2 现状：第一方硬编码加载

- KitKnownModules（src/runner/main.cpp）：constexpr 数组
...
- 加载循环：for 每个 known module → load_kit_module() → modules().emplace(get_key())；
- 启停由 start_enabled_kit_modules() 进行过滤；
...
- **插件开发规范**：[PLUGIN_DEVELOPMENT.md](../../PLUGIN_DEVELOPMENT.md)；
- **历史记录与修复总结**：[fixed.md](../../fixed.md)。