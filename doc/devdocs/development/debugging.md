...
3. Run the following command(don't forget to set the correct platform):
   ```pwsh
   msbuild -restore -p:RestorePackagesConfig=true -p:Platform=ARM64 -m Kit.slnx /tl /p:NuGetInteractive="true"
   ```
...
1. **Clean the solution in Visual Studio**: Build > Clean Solution
   ...
   ```pwsh
   msbuild Kit.slnx /t:Clean /p:Platform=x64 /p:Configuration=Debug
   ```
...
```pwsh
msbuild -restore -p:RestorePackagesConfig=true -p:Platform=x64 -m Kit.slnx
```
