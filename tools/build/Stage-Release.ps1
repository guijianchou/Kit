...
$sourceRoot = Join-Path $repoRoot 'x64\Release'
$destinationRoot = Join-Path $repoRoot "bin\release\$Version"
$reportPath = Join-Path $reportDirectory "Release-$Version.manifest.json"

if (Test-Path -LiteralPath $destinationRoot) {
    if ($Force) {
        Remove-Item -LiteralPath $destinationRoot -Recurse -Force
    } else {
        throw "The $Version delivery directory already exists; use -Force to overwrite it."
    }
}
...
$versionedFiles = @(
    ...
    'Kit.LocalserverModuleInterface.dll', 'Kit.UDPtestModuleInterface.dll',
    ...
    'WinUI3Apps\Kit.AiHub.dll', 'WinUI3Apps\LocalserverLib.dll', 'WinUI3Apps\UDPtestLib.dll'
)
$expectedQuadVersion = "${Version}.0"
foreach ($relative in $versionedFiles) {
    $fullPath = Join-Path $sourceRoot $relative
    if (-not (Test-Path -LiteralPath $fullPath)) {
        Write-Warning "Versioned file not found: $relative"
        continue
    }
...
$sparsePackagePath = Join-Path $destinationRoot 'KitSparse.msix'
$logos = @()
if (Test-Path -LiteralPath $sparsePackagePath) {
    $archive = [IO.Compression.ZipFile]::OpenRead($sparsePackagePath)
    try {
