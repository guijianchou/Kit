...
```powershell
# Restore:
msbuild Kit.slnx -t:restore -p:configuration=debug -p:platform=x64 -m

# Build powertoys slnx
msbuild Kit.slnx -p:configuration=debug -p:platform=x64 -m
...
|------|------------------|-------|
| Clean | `git clean -xdf` (careful) or `msbuild /t:Clean Kit.slnx` | Deep clean removes packages & build outputs |
