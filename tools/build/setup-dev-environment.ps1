<#
.SYNOPSIS
Sets up the development environment for building Kit.

.DESCRIPTION
This script automates the setup of prerequisites needed to build Kit locally:
...
while ($repoRoot -and -not (Test-Path (Join-Path $repoRoot "Kit.slnx"))) {
    ...
    if ($parent -eq $repoRoot) {
        Write-Error "Could not find Kit repository root. Ensure this script is in the Kit repository."
        exit 1
    }
    $repoRoot = $parent
}
...
Write-Host "  1. Open Kit.slnx in Visual Studio"
