...
namespace ManagedCommon
{
    ...
    {
        private const string KitRegistryKey = @"Software\Classes\kit";
        private const string PowerToysRegistryKey = @"Software\Classes\powertoys";
        private const string KitExe = "Kit.exe";
        private const string PowerToysExe = "PowerToys.exe";

        /// <summary>
        /// Gets the Kit installation path by checking registry entries.
        /// </summary>
        ...
        private static string GetPathFromRegistry(RegistryHive hive)
        {
            try
            {
                ...
                string path = GetPathFromProtocolRegistration(baseKey, KitRegistryKey);
                if (!string.IsNullOrEmpty(path))
                {
                    return path;
                }
        ...
        private static string GetPathFromProtocolRegistration(RegistryKey baseKey, string protocolRegistryKey)
        {
            try
            {
                using var key = baseKey.OpenSubKey($@"{protocolRegistryKey}\shell\open\command");
        ...
        private static string GetPathFromCurrentProcess()
        {
            try
            {
                // If we're running inside Kit.exe (dev/debug builds), use the executable location.
                ...
                // As a fallback, walk up from AppContext.BaseDirectory to find Kit.exe or a compatible PowerToys.exe.
        ...
        private static string ExtractPathFromCommand(string command)
        {
            ...
            try
            {
                // Handle quoted paths: "C:\Program Files\Kit\Kit.exe" "%1"
