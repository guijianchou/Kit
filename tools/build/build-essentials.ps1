...
- It will call `RestoreThenBuild 'Kit.slnx'` before building the essential projects to ensure NuGet packages are restored.
...
while ($repoRoot -and -not (Test-Path (Join-Path $repoRoot "Kit.slnx"))) {
    ...
    if ($parent -eq $repoRoot) {
        Write-Error "Could not find Kit repository root."
        exit 1
    }
    $repoRoot = $parent
}
