...
| **持久化数据目录** | `%LOCALAPPDATA%\Kit\AiHub\` | 包括 `AuditHistory\`（审计历史存档）与 `chains\`（任务策略与 Agent 上下文） |
...
│  - EventLogService: Windows Security/System/Application/Setup 日志聚合  │
...
1. **工作流卡片 (Workflow Card)**：
   ...
   - 具备实时进度环、扫描耗时统计与动态取消机制 (`CancellationTokenSource`)。
...
1. **多日志源实时采集**：
   通过 `EventLogService` 高效检索 `Security`、`System`、`Application` 以及 `Setup` 日志，涵盖登录失败（Event 4625）、提权操作（Event 4672）、服务异常挂起（Event 7031）等高危事件。
2. **0-100 安全健康评分 (`HealthScoreCalculator`)**：
   - 基准分 100 分。
   - 按严重等级加权扣分：高危事件 (High) 权重 15 分，中危事件 (Medium) 权重 5 分，低危警告 (Low) 权重 1 分。
   - 动态更新健康度评级（Good: 80~100，Warning: 60~79，Critical: <60）。
3. **交互式审计结果网格与详情弹窗 (`FindingDetailsDialog`)**：
   - 结构化呈现事件发生时间、来源通道、事件 ID、风险等级及一句话摘要。
   - 点击“查看详情”弹窗展示原始事件详情与 AI 建议行动（Suggested Action），支持一键复制诊断信息。
4. **审计历史归档 (`AuditHistoryStorage`)**：
   - 每次审计结果写入 `%LOCALAPPDATA%\Kit\AiHub\AuditHistory\audit_history.json`，支持 30 天自动轮转过期清理，保证历史可溯。
