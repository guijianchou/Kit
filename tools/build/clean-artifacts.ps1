<#
.SYNOPSIS
Cleans Kit build artifacts to resolve build errors.
    ...
    if (Test-Path $vsWhere) {
        ...
        if ($vsPath) {
            ...
            if (Test-Path $msbuildPath) {
                $solutionFile = Join-Path $repoRoot "Kit.sln"
                if (-not (Test-Path $solutionFile)) {
                    $solutionFile = Join-Path $repoRoot "Kit.slnx"
                }
...
Write-Host "  msbuild -restore -p:RestorePackagesConfig=true -p:Platform=x64 -m Kit.slnx"
