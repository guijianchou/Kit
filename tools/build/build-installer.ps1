...
while ($repoRoot -and -not (Test-Path (Join-Path $repoRoot "Kit.slnx"))) {
    $parentDir = Split-Path -Parent $repoRoot
    if ($parentDir -eq $repoRoot) {
        # Reached the root of the drive, Kit.slnx not found
        Write-Error "Could not find Kit repository root. Make sure this script is in the Kit repository."
        exit 1
    }
    $repoRoot = $parentDir
}

if (-not $repoRoot -or -not (Test-Path (Join-Path $repoRoot "Kit.slnx"))) {
    Write-Error "Could not locate Kit.slnx. Please ensure this script is run from within the Kit repository."
    exit 1
}

Write-Host "Kit repository root detected: $repoRoot"
...
if ($Clean) {
    ...
    RunMSBuild 'Kit.slnx' '/t:Clean' $Platform $Configuration
}
...
try {
    ...
    if (-not $SkipBuild) {
        RestoreThenBuild 'Kit.slnx' $commonArgs $Platform $Configuration
    }
