...
- [PLUGIN_DEVELOPMENT.md](PLUGIN_DEVELOPMENT.md) —— Kit 插件与模块开发规范：C++ 契约、模块注册、WinUI 3 + Mica Alt 界面、Logo 图标规范、数据隔离、生命周期及模板工程。
...
- `doc/devdoc/kit-development-experience.md` —— 第一阶段经验总结与后续稳定化检查清单。
- `doc/devdoc/startup-optimization-analysis.md` —— 启动耗时优化深度分析报告（中文）。
- `fix.plan` —— 阶段化插件宿主实施计划（仓库根目录）。
- `fix.md` —— 上游差异与修复记录（仓库根目录）。
- `next.md` —— 同步进度与上游差异检查清单（仓库根目录）。
- [changelog.md](changelog.md) —— 版本历史.

## 构建产物目录

`tools/build/build.ps1` 会按照工程既定配置调用 MSBuild。x64 Debug 构建产物会直接输出到 `x64/Debug/`、`Debug/x64/` 以及各个子项目的自身目录；脚本不会自动把这些产物整理到 `bin/` 下，也不会主动清理它们。

| 路径 | 用途 |
| --- | --- |
| `x64/Debug/` | 主 x64 Debug 运行环境，包含 `Kit.exe` 与 `WinUI3Apps/` |
| `Debug/x64/` 及各项目的 `bin/`、`obj/` | 其它工程输出和中间构建状态 |
| `bin/debug/2.0.23/` | 由 `tools/build/Stage-Debug.ps1` 单独生成并校验的 Debug 测试交付目录 |
...
- 保持运行器构建依赖项与运行时启动的 UI 应用对齐。`Kit.exe` 可以启动并显示托盘图标，即使 `WinUI3Apps\Kit.Settings.exe` 缺失；调试输出可以用陈旧文件隐藏该问题，因此干净的发布验证必须确认设置和快速访问可执行文件都已重新生成。
- 为复制的模块保持 PowerToys/Kit CsWinRT 元数据稳定。`Kit.Interop.winmd` 和 `Kit.GPOWrapper.winmd` 由本机项目发布到 `$(RepoRoot)$(Platform)\$(Configuration)`，`Common.Dotnet.CsWinRT.props` 在先前失败或清理的构建没有留下生成的投影源时使陈旧的 `cswinrt.rsp` 文件无效。这防止导入的模块（如 `Awake` 和快速访问）在其 `Kit.*` 投影重新生成之前编译。
