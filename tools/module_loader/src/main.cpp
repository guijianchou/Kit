...
namespace
{
    void PrintUsage()
    {
        ...
        std::wcout << L"  - Automatically discovers settings from %%LOCALAPPDATA%%\\Kit\\<ModuleName>\\settings.json\n";
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    try
    {
        ...
        if (settingsJson.empty())
        {
            ...
            std::wcerr << L"Expected location: %LOCALAPPDATA%\\Kit\\" << moduleName << L"\\settings.json\n";
            return 1;
        }
