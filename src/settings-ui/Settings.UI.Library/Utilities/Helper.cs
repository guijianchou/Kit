...
namespace Microsoft.PowerToys.Settings.UI.Library.Utilities
{
    ...
    {
        private const string DevEnvironmentMetadataKey = "DevEnvironment";
        private const string LocalDevEnvironment = "Local";
        ...
        public static IFileSystemWatcher GetFileWatcher(string moduleName, string fileName, Action onChangedCallback, IFileSystem fileSystem = null)
        {
            ...
            var path = fileSystem.Path.Combine(LocalApplicationDataFolder(), "Kit", moduleName);
        ...
        {
            var version = productVersion ?? GetProductVersion();
            var environment = (devEnvironment ?? GetDevEnvironment()).Trim();

            if (string.IsNullOrEmpty(environment) || string.Equals(environment, LocalDevEnvironment, StringComparison.OrdinalIgnoreCase))
            {
                return version;
            }
        ...
        {
            return typeof(Helper).Assembly.GetCustomAttributes<AssemblyMetadataAttribute>()
                .FirstOrDefault(attribute => string.Equals(attribute.Key, DevEnvironmentMetadataKey, StringComparison.Ordinal))?.Value ?? string.Empty;
        }
